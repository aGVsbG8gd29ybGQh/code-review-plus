...
**添加评论**：在 Diff 视图中右键 → `Add Comment`（添加评论）。支持选中多行代码作为评论范围，评论最多 500 字符，每条评论自动分配编号。
...
自动评审通过预定义规则批量标记代码块状态，减少手动操作。可以在工具栏点击"自动评审"按钮对全部文件执行，也可以在文件列表中右键选中的文件执行局部自动评审。
...
不需要。只要在 IntelliJ IDEA 中使用插件，Spring 规则开箱即用。插件通过分析项目中的 Spring 注解和 Bean 定义来构建依赖图，无需依赖 Spring Boot 插件或运行时环境。
