...
### AI 评审 [Premium] ✨ New in v4.0

基于 **Claude Code CLI + MCP Server** 架构，让 AI 自主完成代码评审。插件在本地启动一个 MCP HTTP Server，Claude Code CLI 子进程通过工具调用从 Server 获取 diff 内容，自主读取源文件，发现问题后提交。整个过程无需人工干预。
...
> **前提条件**：需要安装 Claude Code CLI 并完成登录。详见 [AI 评审](#ai-评审-premium) 章节。
...
| <img src="images/run.svg" width="16" height="16">     | AI 评审 | 启动 AI 评审，调用 Claude Code CLI 对所有未评审代码块进行智能评审  |
...
AI 评审利用 **Claude Code CLI** 结合插件内嵌的 **MCP Server** 实现完全自主的代码评审。三个专家 Agent 并行运行，覆盖规范、安全和 SQL 三个维度，自动发现人工容易遗漏的深层问题。

### 前置条件

使用 AI 评审功能前，需要确认以下几点已就绪：

**1. 本地 Claude Code CLI 环境已配置妥当**

请参照 [Claude Code 官方文档](https://docs.anthropic.com/zh-CN/docs/claude-code/getting-started) 完成安装和登录。
...
若上述命令执行正常，说明环境已就绪，插件可以直接使用。

**3. 在插件设置中验证连通性**

进入 `Settings → Tools → Code Review Plus → AI Review`，在"Claude 启动命令"一栏点击**验证**按钮，插件会执行 `claude --version` 检测命令是否可达。

如果命令在终端可用但插件验证失败，通常是 IDEA 的 `PATH` 环境变量与终端不一致导致的。此时可以在"Claude 启动命令"中填入 claude 的完整路径（如 `/usr/local/bin/claude`）。
...
|-------------|-----------------------------------------------------------------------------------------|-----------|
| 启用 AI 评审    | 全局开关                                                                                    | 启用        |
| Claude 启动命令 | CLI 可执行文件路径，若 claude 命令不在 PATH 中可填写完整路径                                                 | `claude`  |
| 模型          | 指定使用的 Claude 模型（如 `claude-opus-4-5`），留空时使用 CLI 默认模型                                     | 空（CLI 默认） |
| Prompt 语言   | 评审问题输出语言，`zh` 为中文，`en` 为英文，输入的 prompt 和给 AI 指定的输出语言受此设置项影响，请设置为自己常用语言                   | 跟随 IDE 语言 |
...
| AI 评审配置           | 配置 Claude Code CLI 命令、模型、专家开关、自定义规则等，详见 AI 评审章节 | 见上文     |
...
3. **运行 AI 评审** → 让 AI 自动发现规范、安全、SQL 问题，结果自动写入数据库（需要 Claude Code CLI）
4. **按文件逐个评审** → 从未评审的文件开始，双击打开 Diff 视图，重点关注 AI 未覆盖或标记为问题的部分
5. **标记状态 + 写评论** → 通过的标记通过，有问题的标记问题并写评论
...
| 维度 | 自动评审 | AI 评审 |
|------|--------|--------|
| 实现方式 | 基于预定义规则的静态分析 | Claude Code AI 大模型推理 |
| 评审速度 | 秒级 | 分钟级 |
